;(function () {
  ...
  function TransactionBuilderService ($timeout, $q, networkService, accountService, ledgerService, gettextCatalog, utilityService) {
    ...
    function createTransaction (deferred, config, fee, createTransactionFunc, setAdditionalTransactionPropsOnLedger) {
      ...
      if (config.ledger) {
        ...
        ledgerService.signTransaction(config.ledger, transaction)
          .then(({ signature }) => {
            ...
            transaction.id = sth.crypto.getId(transaction)
      ...
      if (sth.crypto.getAddress(transaction.senderPublicKey, networkService.getNetwork().version) !== config.fromAddress) {
      ...
      return new Promise((resolve, reject) => {
        accountService.getFees(false).then(fees => {
          const invalidAddress = transactions.find(t => {
            return !sth.crypto.validateAddress(t.address, network.version)
          })
            ...
            transactions.map(({ address, amount, smartmessage }, i) => {
              return new Promise((resolve, reject) => {
                ...
                if (ledger) {
                  ...
                  if (sth.crypto.getAddress(transaction.senderPublicKey, network.version) !== fromAddress) {
    ...
    function createSecondPassphraseCreationTransaction (config) {
      return prepareTransaction(config, (deferred, account, fees) => {
        ...
        createTransaction(deferred,
                          ...
                          () => sth.signature.createSignature(config.masterpassphrase, config.secondpassphrase, fees.secondsignature))
      })
    }
    ...
    function createDelegateCreationTransaction (config) {
      return prepareTransaction(config, (deferred, account, fees) => {
        ...
        createTransaction(deferred,
                          config,
                          fees.delegate,
                          () => sth.delegate.createDelegate(config.masterpassphrase, config.username, config.secondpassphrase, fees.delegate))
      })
    }
    ...
    function createVoteTransaction (config) {
      return prepareTransaction(config, (deferred, account, fees) => {
        ...
        createTransaction(deferred,
                          config,
                          fees.vote,
                          () => sth.vote.createVote(config.masterpassphrase, config.publicKeys.split(','), config.secondpassphrase, fees.vote),
