;(function () {
  ...
  angular.module('sthclient.accounts')
    .service('accountService', ['$q', '$http', 'networkService', 'storageService', 'ledgerService', 'gettextCatalog', 'gettext', 'utilityService', 'STH_LAUNCH_DATE', AccountService])
   ...
   * @constructor
   */
  function AccountService ($q, $http, networkService, storageService, ledgerService, gettextCatalog, gettext, utilityService, STH_LAUNCH_DATE) {
    const self = this
    ...
    function showTimestamp (timestamp) { // eslint-disable-line no-unused-vars
      const date = utilityService.sthStampToDate(timestamp)
    ...
    function createAccount (passphrase) {
      return new Promise((resolve, reject) => {
        const publicKey = sth.crypto.getKeys(passphrase).publicKey
        const address = sth.crypto.getAddress(publicKey, networkService.getNetwork().version)
    ...
    function formatTransaction (transaction, recipientAddress) {
      ...
      transaction.date = utilityService.sthStampToDate(transaction.timestamp)
    ...
    function isValidAddress (address) {
      return sth.crypto.validateAddress(address, networkService.getNetwork().version)
    }
    ...
    function createVirtual (passphrase) {
      const deferred = $q.defer()
      const address = sth.crypto.getAddress(sth.crypto.getKeys(passphrase).publicKey, networkService.getNetwork().version)
