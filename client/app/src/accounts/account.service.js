;(function () {
  ...
  angular.module('sthclient.accounts')
    .service('accountService', ['$q', '$http', 'networkService', 'storageService', 'ledgerService', 'gettextCatalog', 'SATOSHI_UNIT', AccountService])
  ...
  function AccountService ($q, $http, networkService, storageService, ledgerService, gettextCatalog, SATOSHI_UNIT) {
    ...
    function getTransactionLabel (transaction, recipientAddress = null) {
      ...
      if (recipientAddress && transaction.recipientId === recipientAddress && transaction.type === 0) {
        label = gettextCatalog.getString('Receive STH')
      }
    ...
    function formatTransaction (transaction, recipientAddress) {
      ...
      if (transaction.recipientId === recipientAddress) {
        ...
        //   transaction.label = gettextCatalog.getString("Receive STH")
        // }
      }
      ...
      transaction.humanTotal = numberToFixed(transaction.total / SATOSHI_UNIT) + ''
      ...
      getFees().then(function (fees) {
        ...
        if (type === 0) { // send sth
          ...
          if (config.amount + fees.send > account.balance) {
            deferred.reject(gettextCatalog.getString('Not enough STH on your account ') + config.fromAddress)
            return deferred.promise
          }
          ...
          if (account.balance < fees.secondpassphrase) {
            deferred.reject(gettextCatalog.getString('Not enough STH on your account ') + config.fromAddress + ', ' + gettextCatalog.getString('you need at least 5 STH to create a second passphrase'))
            return deferred.promise
          }
          try {
          ...
          if (account.balance < fees.delegate) {
            deferred.reject(gettextCatalog.getString('Not enough STH on your account ') + config.fromAddress + ', ' + gettextCatalog.getString('you need at least 55 STH to register delegate'))
            return deferred.promise
          }
          console.log(config)
          try {
          ...
          if (account.balance < fees.vote) {
            deferred.reject(gettextCatalog.getString('Not enough STH on your account ') + config.fromAddress + ', ' + gettextCatalog.getString('you need at least 1 STH to vote'))
            return deferred.promise
          }
          try {
            ...
            if (virtual[folder]) {
              if (arguments.length === 1) {
                if (value === null) {
                  ...
                  virtual[folder].amount = value * SATOSHI_UNIT
                }
              } else {
                return virtual[folder].amount === null ? '' : virtual[folder].amount / SATOSHI_UNIT
              }
            }
          }
        }
